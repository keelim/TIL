...
- [웹 기술](#웹-기술)
  - [REST API](#rest-api)
    - [REST (Representational State <u>Transfer) 기본](#rest-representational-state-utransfer-기본)
  - [HTTP status code](#http-status-code)
    - [200번대 : 통신 성공](#200번대--통신-성공)
    - [300번대 : 리다이렉트](#300번대--리다이렉트)
    - [400번대 : 클라이언트 오류](#400번대--클라이언트-오류)
    - [500번대 : 서버 오류](#500번대--서버-오류)
  - [쿠키 & 세션](#쿠키--세션)
  - [저장 위치](#저장-위치)
  - [만료 시점](#만료-시점)
  - [리소스](#리소스)
  - [용량 제한](#용량-제한)
  - [Web Server와 WAS의 차이](#web-server와-was의-차이)
    ...
    - [WAS](#was)
      - [역할](#역할)
      - [WAS 주요 기능](#was-주요-기능)
      - [웹 서버가 필요한 이유](#웹-서버가-필요한-이유)
      - [WAS가 필요한 이유](#was가-필요한-이유)
      - [그러면 WAS로 웹 서버 역할까지 다 처리할 수 있는거 아닌가요?](#그러면-was로-웹-서버-역할까지-다-처리할-수-있는거-아닌가요)
      - [가장 효율적인 방법](#가장-효율적인-방법)
  - [API Key](#api-key)
  - [동작방식](#동작방식)
  - [문제점](#문제점)
  - [OAuth2](#oauth2)
  ...
  - [OAuth](#oauth)
    ...
    - [브라우저 주요 기능](#브라우저-주요-기능)
    - [브라우저 기본 구조](#브라우저-기본-구조)
    - [사용자 인터페이스](#사용자-인터페이스)
    - [브라우저 엔진](#브라우저-엔진)
    - [렌더링 엔진](#렌더링-엔진)
    - [통신](#통신)
...
REST : 웹 (HTTP/HTTPS) 의 장점을 활용한 아키텍쳐

### REST (Representational State <u>Transfer) 기본
...
- REST 특징
  ...
  - Uniform Interface
    ...
    - HTTP 표준만 맞는다면, 어떤 기술도 가능한 Interface 스타일 REST API 정의를 HTTP + JSON로 하였다면, C, Java, Python, IOS 플랫폼 등 특정 언어나 기술에 종속 받지 않고, 모든 플랫폼에 사용이 가능한 Loosely Coupling 구조 포함
...
클라우드 환경에서 HTTP API를 통해 통신하는 것이 대부분임 이때, 응답 상태 코드를 통해 성공/실패 여부를 확인할 수 있으므로 API 문서를 작성할 때 꼭 알아야 할 것이 HTTP status code다
...
바뀌지 않는 페이지 웹 서버는 파일 경로 이름을 받고, 경로와 일치하는 file contents를 반환함
...
인자에 따라 바뀌는 페이지, 인자의 내용에 맞게 동적인 contents를 반환함
...
API Key를 사용자에게 직접 발급하고 해당 Key를 통해 통신을 하기 때문에 통신구간이 암호화가 잘 되어 있더라도 Key가 유출된 경우에 대비하기 힘들다. 그렇기때문에 주기적으로 Key를 업데이트를 해야하기 때문에 번거롭고 예기치 못한 상황(한쪽만 업데이트가 실행되어 서로 매치가 안된다는 등)이 발생할 수 있다. 또한, Key한가지로 정보를 제어하기 때문에 보안문제가 발생하기 쉬운편이다.
...
(우리가 웹페이지에 접속할 때 한꺼번에 뜨지 않고 점점 화면에 나오는 것이 이 때문!!!)s
